...
PDF_PATH = "8G GalarDex + Armor_Crown.pdf"   # ← adapte si besoin
OUT_JSON = "../../ptu/data/pokedex/pokedex_8g.json"
OUT_NDJSON = "../../ptu/data/pokedex/pokedex_8g.ndjson"
...
def parse_moves_list(lines, start_idx):
    ...
    while i < len(lines):
        ...
        if re.search(r'(?i)^Move List$', line):
            i += 1
            continue
        if re.search(r'(?i)^Level Up Move List$', line) or (line == "Level Up" and nxt == "Move List"):
            current = "Level Up Move List"
            i += 2 if (line == "Level Up" and nxt == "Move List") else 1
            continue
        if re.search(r'(?i)^TM/HM Move List$', line) or (line == "TM/HM" and nxt == "Move List"):
            current = "TM/HM Move List"
            i += 2 if (line == "TM/HM" and nxt == "Move List") else 1
            continue
        if re.search(r'(?i)^Egg Move List$', line) or (line == "Egg" and nxt == "Move List"):
            current = "Egg Move List"
            i += 2 if (line == "Egg" and nxt == "Move List") else 1
            continue
        if re.search(r'(?i)^Tutor Move List$', line) or (line == "Tutor" and nxt == "Move List"):
            current = "Tutor Move List"
            i += 2 if (line == "Tutor" and nxt == "Move List") else 1
            continue
        if re.search(r'(?i)^Mega Evolution', line):
            break
        if re.search(r'(?i)^(Base Stats:|Basic Information|Evolution:|Size Information|Breeding Information|Diet|Habitat|Capability List|Skill List|Mega Evolution)$', line):
            break
        if current:
            sections[current].append(line)
        i += 1
...
def extract_page(page_text: str, page_index: int):
    ...
    cap_block = collect_between(idx_cap, next_all)
    if cap_block:
        # Join lines, re-inserting commas between tokens that were split across lines inside parentheses.
        acc = ""
        level = 0
        for j, raw in enumerate(cap_block):
            l = clean_line(raw)
            if not l:
                continue
            # If we're inside parentheses and previous char isn't a comma or opening paren,
            # and current line doesn't start with ')' or ',', insert a comma before this chunk.
            needs_comma = (level > 0 and acc and not acc.rstrip().endswith((',', '('))
                           and not l.startswith((')', ',')))
            if needs_comma:
                acc += ', ' + l
            else:
                # default: add space separator
                if acc and not acc.endswith(' '):
                    acc += ' '
                acc += l
            # update parentheses depth from this chunk
            level += l.count('(') - l.count(')')

        if level != 0:
            logger.warning(f"[p{page_index} {species}] Capability parentheses look unbalanced after join (level={level}).")

        # Now split only on commas that are OUTSIDE parentheses
        parts = split_commas_outside_parens(acc)
        caps = [clean_line(p) for p in parts if clean_line(p)]
        record["Capabilities"] = caps
    else:
